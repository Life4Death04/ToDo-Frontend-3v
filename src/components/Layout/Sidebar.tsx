  ...
  return(
    <nav className="fixed top-0 left-0 h-screen w-12 py-3 sm:w-20 lg:w-64 lg:px-2 bg-gray-200 flex flex-col">
      ...
      <ul className="flex flex-col h-full gap-4 w-full xsm:items-center lg:items-start">
        <SidebarLinkItem linkText="My Tasks" linkUrl="/settings" classIcon="fa-user" hoverEffect={true}/>
        <SidebarLinkItem linkText="Shared Tasks" linkUrl="/settings" classIcon="fa-user" hoverEffect={true}/>
        <SidebarLinkItem linkText="Archived Tasks" linkUrl="/settings" classIcon="fa-user" hoverEffect={true}/>
      </ul>
      ...
      <footer className="mt-auto w-full lg:px-2 lg:py-3 lg:border-t-1 border-t-black/50">
        <ul className="flex flex-col gap-4 lg:items-start">
          <SidebarLinkItem linkText="Settings" linkUrl="/settings" classIcon="fa-gear" hoverEffect={true}></SidebarLinkItem>
          <SidebarLinkItem linkText="Log Out" linkUrl="/logout" classIcon="fa-arrow-right-from-bracket" hoverEffect={true}></SidebarLinkItem>
        </ul>
      </footer>
    </nav>
  );
  ...
  return(
    <Link to={linkUrl} className="w-full">
      <li className={`flex justify-center gap-2 py-2 lg:justify-start lg:px-4 ${hoverEffect && 'hover:bg-white hover:rounded-3xl hover:shadow-md'}`}>
        <i className={`fa-solid ${classIcon} sm:text-2xl md:text-2xl`}></i>
